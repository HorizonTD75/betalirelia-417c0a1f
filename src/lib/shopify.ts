...
const SHOPIFY_STORE_PERMANENT_DOMAIN = 'umrhpe-ta.myshopify.com';
const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
const SHOPIFY_STOREFRONT_TOKEN = '1c503fb30d5c7ddbbdb2547f719ec1b6';
...
export async function storefrontApiRequest(query: string, variables: Record<string, unknown> = {}) {
  ...
  if (response.status === 402) {
    toast.error("Shopify : paiement requis", {
      description: "L'accès à l'API Shopify nécessite un plan actif.",
    });
    return;
  }
